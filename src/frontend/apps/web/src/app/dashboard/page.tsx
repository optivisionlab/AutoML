        ...
        {metrics.map((metric) => {
          ...
          return (
            <Link
              key={metric.label}
              href={metric.href}
              className="group rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md dark:border-white/10 dark:bg-[#0b121e]"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-2 truncate text-sm font-bold text-slate-700 dark:text-slate-300">
                    {metric.change}
                  </p>
                ...
                <div
                  className={cn(
                    "flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl shadow-xs transition group-hover:scale-105",
                    metric.tone,
                  )}
                >
        ...
        <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/10">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[620px] text-left text-sm">
              <thead className="bg-slate-50 text-[11px] font-black uppercase text-slate-400 dark:bg-white/5">
                <tr>
                  <th className="px-4 py-3">{t("table.dataset")}</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/10">
                {dashboard.recentJobs.length > 0 ? (
                  dashboard.recentJobs.map((job) => {
                    ...
                    const href = done
                      ? `/training-history/${job.job_id}`
                      : `/training-history?highlight=${job.job_id}`;

                    return (
                      <tr
                        key={job.job_id}
                        tabIndex={0}
                        className="transition-colors hover:bg-slate-50/80 dark:hover:bg-white/5 focus-visible:bg-blue-50/60 dark:focus-visible:bg-blue-950/30 outline-none text-slate-600 dark:text-white/70"
                      >
                        <td className="px-4 py-3 font-black text-slate-800 dark:text-white">
                          <Link href={href} className="hover:text-automl-blue hover:underline">
                            {job.data?.name || common("unknown")}
                          </Link>
                        </td>
                        <td className="px-4 py-3 font-medium">{done ? job.best_model || common("unknown") : t("processing")}</td>
                        <td className="px-4 py-3">{formatDate(job.create_at, dateLocale, common("noData"))}</td>
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-10 text-center font-semibold text-slate-500">
                      {t("emptyTrainingRuns")}
                    </td>
                  </tr>
                )}
