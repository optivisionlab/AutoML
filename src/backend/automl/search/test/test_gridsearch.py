...
def run_test(dataset_name='glass'):
    """Run the grid search test with a specified dataset.
    ...
    # Initialize the grid search strategy
    opt = GridSearchStrategy(
        cv=5,
        scoring=None,  # Use default scoring metrics
        metric_sort='f1',  # Sort by F1 score
        n_jobs=-1,
        verbose=1,
        random_state=42
    )
    ...
    all_results = []
    output_file = 'gridsearch_test_results.csv'
...
if __name__ == "__main__":
    ...
    print(f"Running Grid Search test with dataset='{dataset}'")
